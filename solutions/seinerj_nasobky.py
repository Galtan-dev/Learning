...
while END_OF_INTERVAL != 0:
    ...
    if (END_OF_INTERVAL % 3) == 0 and (END_OF_INTERVAL % 5) != 0 and (END_OF_INTERVAL < 1234):
    ...
    elif (END_OF_INTERVAL % 5) == 0 and (END_OF_INTERVAL % 3) != 0 and (END_OF_INTERVAL < 1234):
        # variable to which are collected numbers saved
        END_CONDITION += END_OF_INTERVAL
        # counter
        END_OF_INTERVAL -= 1
    # third condition that collect numbers dividable by 5 and 3 and lower that biggest number
    elif (END_OF_INTERVAL % 5) == 0 and (END_OF_INTERVAL % 3) == 0 and (END_OF_INTERVAL < 1234):
        # variable to which are collected numbers saved
        END_CONDITION += END_OF_INTERVAL
        # counter
